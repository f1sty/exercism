defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    ...
    |> then(&Enum.at(Stream.cycle(scale), &1 + @step_shift[step]))
  end
  ...
  def scale(tonic, pattern) do
    ...
    initial = String.capitalize(tonic)

    pattern
    |> String.graphemes()
    |> Enum.reduce([initial], fn step, [tonic | _] = acc -> [step(scale, tonic, step) | acc] end)
    |> Enum.reverse()
  end

  defp get_scale(scale, tonic) do
    tonic = String.capitalize(tonic)
    shift = Enum.find_index(scale, &(&1 == tonic))
